...
def make_label():
    ...
    text = tkinter.Label(output_frame, text=text_entry.get(), bg="#cc6666")
    text.pack()
...
def count_up(number):
    ...
    text = tkinter.Label(output_frame, text=number, bg="#cc6666")
    text.pack()

    value = number + 1

# Define frames
input_frame = tkinter.Frame(root, bg="#ffcccc", width=500, height=100)
output_frame = tkinter.Frame(root, bg="#cc6666", width=500, height=400)
...
count_button.grid(row=1, column=0, columnspan=2, pady=5, sticky="WE")
